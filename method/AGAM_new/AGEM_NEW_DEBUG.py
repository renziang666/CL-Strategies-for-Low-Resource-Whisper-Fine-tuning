...
class AGEMtrainer(Seq2SeqTrainer):
    def __init__(self, replay_buffer, buffer_batch_size=4, effective_buffer_batch_size=24, physical_buffer_batch_size=4, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.replay_buffer = replay_buffer
        # buffer_batch_size现在用于构建单个回放批次的总大小
        # 我们期望的、等效的回放批次大小
        self.effective_buffer_batch_size = effective_buffer_batch_size
        # 每次实际能放入显存的物理批次大小
        self.physical_buffer_batch_size = physical_buffer_batch_size
        self.buffer_batch_size = buffer_batch_size
        print(f"✅ AGEMtrainer 初始化成功！回放批次大小: {self.buffer_batch_size}")
...
# 确保删掉旧的 calculate_reference_gradient 函数
    ...
    def calculate_reference_gradient(self):
        ...
        # 1. 采样足够多的样本用于整个等效大批次
        total_samples_needed = min(self.effective_buffer_batch_size, len(self.replay_buffer))
        replay_samples = random.sample(self.replay_buffer, total_samples_needed)
        
        # 2. 手动执行梯度累积
        self.model.zero_grad() # 累积前清零梯度
        ...
        for i in range(accumulation_steps):
            ...
            # 关键：对loss进行缩放，以保证梯度量级正确
            # 如果不缩放，最终需要除以 accumulation_steps
            loss = loss / accumulation_steps
            
            # 反向传播，梯度会自动累加到 .grad 属性上
            self.accelerator.backward(loss)

        # 3. 累积完毕后，从 .grad 属性中提取完整的梯度向量
        ref_grad = self._get_grad_vector()
        self.model.zero_grad() # 清理梯度，以免影响下一步
...
if __name__ == "__main__":
    ...
    # 你的参数解析保持不变...
    parser.add_argument("--model_id", default="/data/share/guodong/workspace/models/whisper_hf/whisper-medium/whisper-medium")
    ...
    trainer = AGEMtrainer(
        ...
        effective_buffer_batch_size=24,  # 你想要模拟的大批次大小
        physical_buffer_batch_size=4,    # 你确定能放进显存的小批次大小
        model=model,
        args=training_args,
