...
class AGEMtrainer(Seq2SeqTrainer):
    ...
    def training_step(self, model, inputs, num_items_in_batch=None):
        ...
        for lang in self.protected_languages:
            ...
            ref_grads_list.append(g_ref_task)
            
        # 3. 如果计算出了任何参考梯度，则对它们求平均
        if ref_grads_list:
            g_ref_avg = torch.mean(torch.stack(ref_grads_list), dim=0)
            
            # 4. (关键) 恢复当前任务的梯度到模型中，以便进行投影
            self._set_grad_vector(g_current)
            
            # 5. 执行投影
            g_projected = self._project_to_half_space(g_current, g_ref_avg)
            
            # 6. 将最终（可能被投影的）梯度设置回模型
            self._set_grad_vector(g_projected)
        else:
            # 如果没有参考梯度，则直接使用当前任务的梯度
            self._set_grad_vector(g_current)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", default="/data/share/guodong/workspace/models/finetune_whisper_trained/medium_AGEM_old92")
