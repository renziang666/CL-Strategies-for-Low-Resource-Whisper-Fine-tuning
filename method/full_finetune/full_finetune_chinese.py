...
def setup_training_args(args):
    return Seq2SeqTrainingArguments(
        output_dir=f"/data/share/guodong/workspace/models/finetune_whisper_trained/chinese_encoder_cv_full_v3",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--processed_data_root",  default="/data/share/guodong/workspace/datasets/FLEURS/chinese_cache_commonvoice")
    ...
    # 确认关键字段是否存在且合理
    # 1. 'input_features' 是否是一个浮点数张量？
    print(f"\n input_features 的类型: {type(sample['input_features'])}, 形状: {sample['input_features'].shape}")

    # 2. 'labels' 是否是一个整数列表或张量？
    print(f" labels 的类型: {type(sample['labels'])}, 内容: {sample['labels']}")

    # 3. 'language' 字段是否存在且值正确？
    if "language" in sample:
        print(f" language 字段的值: '{sample['language']}'")
    else:
        print("⚠️ 警告：数据中缺少 'language' 字段！")
    # 合并数据集
